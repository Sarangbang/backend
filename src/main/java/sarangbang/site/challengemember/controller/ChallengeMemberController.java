...
public class ChallengeMemberController {
     ...
     * 챌린지 멤버 목록 조회 API
     */
    @Operation(summary = "챌린지 멤버 목록 조회", description = "챌린지 가입 멤버 목록을 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "챌린지 멤버 목록 조회 성공"),
            @ApiResponse(responseCode = "400", description = "챌린지 멤버 목록 조회 실패", content = @Content(mediaType = "application/json"))
    })
    @GetMapping("/{challengeId}")
    public ResponseEntity<List<ChallengeMemberDTO>> getMembersByChallengeId(
            @PathVariable Long challengeId,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        String userId = userDetails.getId();
        try {
            List<ChallengeMemberDTO> responseDto = challengeMemberService.getMembersByChallengeId(challengeId, date, userId);
